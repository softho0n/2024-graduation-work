def db_add_like_music(subscription_collection, username, music_title):
    subscription_collection.update_one({"username": username}, {"$addToSet": {"likes": music_title}}, upsert=True)


def db_delete_like_music(subscription_collection, username, music_title):
    subscription_collection.update_one({"username": username}, {"$pull": {"likes": music_title}})
